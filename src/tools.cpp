...
double total_threshold_l = 0;
double in_threshold_l = 0;
double total_threshold_r = 0;
double in_threshold_r = 0;
...
lmarker Tools::lidarSense(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize)
{
	...
	if(percentage)
	{
		if(car.ukf.NIS_l_ < 5.991)
		{
			in_threshold_l += 1;
		}
		total_threshold_l += 1;
		std::cout << "percentage_l: " << in_threshold_l/total_threshold_l*100 << "%" << std::endl;
	}
...
rmarker Tools::radarSense(Car& car, Car ego, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize)
{
    ...
    car.ukf.ProcessMeasurement(meas_package);
  	...
  	if(percentage)
	{
		if(car.ukf.NIS_r_ < 7.815)
		{
			in_threshold_r += 1;
		}
		total_threshold_r += 1;
		std::cout << "percentage_r: " << in_threshold_r/total_threshold_r*100 << "%" << std::endl;
	}
